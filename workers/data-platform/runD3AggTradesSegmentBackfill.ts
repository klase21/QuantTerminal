...
async function main() {
  ...
  try {
    ...
    if (!selected.length) { console.log(JSON.stringify({ status: "NO_ELIGIBLE_PARTITIONS" })); return }
    const postgresFreeBytes = Number(arg("--postgres-free-bytes") ?? "0"); const fs = await statfs(process.env.D3_BACKFILL_OBJECT_ROOT!)
    const capacity = evaluateAggTradesSegmentCapacity({ snapshot: value, postgresFreeBytes, artifactFreeBytes: Number(fs.bavail) * Number(fs.bsize), sourceBytesToProcess: selected.reduce((sum, partition) => sum + partition.compressedBytes, 0), partitionCountToProcess: selected.length, additionalArtifactGrowthBytes: ADDITIONAL_ARTIFACT_GROWTH_BYTES, additionalPostgresGrowthBytes: AGG_TRADES_REMAINING_DATASET_STORAGE_BYTES })
    if (postgresFreeBytes <= 0 || capacity.status !== "PASS") throw new Error(`SEGMENT_CAPACITY_NOT_APPROVED:${JSON.stringify(capacity)}`)
    console.log(JSON.stringify({ status: "BOUNDED_CAPACITY_PASS", capacity }))

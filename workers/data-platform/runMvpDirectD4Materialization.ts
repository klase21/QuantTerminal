...
function d4Environment(value: Record<string,string>) { return { D4_ISOLATED_POSTGRES_URL:value.D4_URL,D4_EXPECTED_DATABASE_NAME:value.D4_DB,D2_CANONICAL_POSTGRES_URL:value.CORE_URL,MVP_REFRESH_ISOLATED_POSTGRES_URL:value.REFRESH_URL,MVP_SERVING_ISOLATED_POSTGRES_URL:value.SERVING_URL } }
...
function refreshClient(value:Record<string,string>){return new MvpRefreshPostgresClient(value.REFRESH_URL,{MVP_REFRESH_ISOLATED_POSTGRES_URL:value.REFRESH_URL},{database:value.REFRESH_DB,role:value.REFRESH_ROLE})}
function servingClient(value:Record<string,string>,intent:"MIGRATION_OWNER"|"PUBLISHER"){return new MvpServingPostgresClient(value.SERVING_URL,intent,{MVP_SERVING_ISOLATED_POSTGRES_URL:value.SERVING_URL},"LOCAL_ISOLATED",{database:value.SERVING_DB,role:value.SERVING_ROLE})}
...
async function verifyCore(core: Awaited<ReturnType<typeof openCore>>) {
  const rows = await core.sql.unsafe<Array<{
    ...
    SELECT
      (SELECT count(*)::text FROM canonical.ohlcv) AS "ohlcv",
      (SELECT count(*)::text FROM canonical.open_interest) AS "openInterest",
      (SELECT count(*)::text FROM canonical.funding) AS "funding",
      (SELECT count(*)::text FROM canonical.stream_manifests WHERE source_dataset_id = 'agg-trade' AND validation_status = 'VALIDATED') AS "manifests",
      (SELECT count(*)::text FROM control.canonical_commits) AS "commits",
      (SELECT count(*)::text FROM population.candidates) AS "candidates",
      (SELECT count(*)::text FROM raw.objects) AS "objects",
...
async function full(value: Record<string, string>) {
  ...
  try {
    ...
    for (const projection of result.projections) {
      members.push(
        Object.freeze({
          ...
          memberId: projection.projectionVersionId,
          memberChecksum: projection.projectionChecksum,
          canonicalSortKey: `PROJECTION:${projection.projectionKind}:${projection.subjectId}:${projection.projectionVersionId}`,
          inheritedSourceCorpusId: null,
          schemaVersion: projection.schemaVersion,
          metadata: Object.freeze({ freshD4: true }),
        }),
      );
    }
    for (const evidence of result.evidenceResults) {
      members.push(
        Object.freeze({
          memberKind: "EVIDENCE_SUMMARY",
          memberId: evidence.packet!.packetVersionId,
          memberChecksum: evidence.packet!.packetChecksum,
          canonicalSortKey: `EVIDENCE:${evidence.assessment.instrument}:${evidence.packet!.packetVersionId}`,
          inheritedSourceCorpusId: null,
          schemaVersion: "mvp8e-evidence/1.0.0",
          metadata: Object.freeze({ freshD4: true }),
        }),
      );
    }
    for (const replay of replays) {
      members.push(
        Object.freeze({
          memberKind: "REPLAY_SNAPSHOT",
          memberId: `mvp8e-replay:${replay.sourceProjectionVersionId}`,
          memberChecksum: replay.modelChecksum,
          canonicalSortKey: `REPLAY:${replay.instrument}:${replay.sourceProjectionVersionId}`,
          inheritedSourceCorpusId: null,
          schemaVersion: replay.modelVersion,
          metadata: Object.freeze({ sampleCounts: replay.sampleCounts }),
        }),
      );
    }

    const serving = servingClient(value, "PUBLISHER");
    try {
      await serving.verify();
      const canonicalMembers = canonicalizeServingCorpusMembers(members);
      const schemaVersion = "mvp-serving/1.0.0";
      const checksum = computeCandidateServingChecksum({
        governedThrough: END,
        schemaVersion,
        members: canonicalMembers,
      });
      const corpusId = `mvp8e-candidate:${checksum}`;
      const assembled = await new LocalInactiveCandidateAssemblyService(
        serving,
      ).assembleGenesis({
        candidate: {
          corpusId,
          sourceCorpusId: result.sourceCorpus.corpusId,
          sourceCorpusChecksum: result.sourceCorpus.corpusChecksum,
          governedThrough: END,
          schemaVersion,
          generatedAt: new Date().toISOString(),
          members: canonicalMembers,
          limitations: Object.freeze([
            "INACTIVE_LOCAL_CANDIDATE",
            "MVP8E_DIRECT_D4_MATERIALIZATION",
          ]),
        },
      });
      process.stdout.write(
        JSON.stringify({
          ...
          datasetWatermarks: 4,
          commonWatermark: END,
          candidateCorpusId: corpusId,
          candidateChecksum: assembled.servingChecksum,

...
const historicalSql = D4_MIGRATION_ORDER.filter((name) => !name.startsWith("007_") && !name.startsWith("008_")).map((name) => readFileSync(path.join(root, name), "utf8")).join("\n")
...
export const noExecutableRoleDdl = !/CREATE ROLE|CREATE USER/i.test(historicalSql) && /CREATE ROLE qt_d4_consistency_worker|CREATE ROLE qt_d4_read_only|CREATE ROLE qt_d4_evidence_assembler/i.test(sql)

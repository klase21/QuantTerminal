...
`mvp_green_migration_owner` has no login credential to capture or store. Migration execution therefore requires a separate LOGIN principal with explicit membership in the NOLOGIN owner role before `SET ROLE` can be used. Membership is not granted during role creation, database creation, or migration execution and must be introduced as a separately approved governed operation, proposed as `GREEN_MIGRATION_ROLE_MEMBERSHIP_GRANT`. Until that gate is implemented and executed, database creation and migrations must not assume that membership exists.

...
The process-environment bootstrap instantiates the concrete dataset, persistence, downstream, Replay, and inactive candidate adapters and owns their disposal. Durable D2/D3 come only from the certified integrated factory; D4, refresh, and serving retain their isolated boundaries. The authenticated no-write preflight passes, the worker no longer injects ports, and fixture ports are never substituted for live execution.

...
check("approved migration discovery", migrations.length === 6)
check("migration checksums deterministic", migrations.every((migration) => /^[a-f0-9]{64}$/.test(migration.checksum)))
check("migration order deterministic", migrations.map((migration) => migration.migrationId).join(",") === "001,002,003,004,005,006")

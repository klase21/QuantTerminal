  ...
  try {
    ...
    return await withServingPostgresFacade(async (_facade, context, port) => {
      if (context.selection === "PREVIEW_INACTIVE_CANDIDATE") {
        const integrity = context.previewIntegrity as { readonly counts?: { readonly projections?: number; readonly evidenceSummaries?: number; readonly replaySnapshots?: number; readonly members?: number }; readonly exposureCount?: number } | undefined
        const healthy = context.transactionReadOnly && integrity?.counts?.projections === 62 && integrity.counts.evidenceSummaries === 6 && integrity.counts.replaySnapshots === 6 && integrity.counts.members === 74 && integrity.exposureCount === 0
        return NextResponse.json({ status: healthy ? "HEALTHY" : "UNHEALTHY", servingMode: context.mode, databaseConnectivity: "CONNECTED", activeCorpusId: context.corpusId, activeServingChecksum: context.checksum, governedThrough: context.governedThrough, projectionCount: integrity?.counts?.projections, evidenceSummaryCount: integrity?.counts?.evidenceSummaries, replaySnapshotCount: integrity?.counts?.replaySnapshots, manifestMemberCount: integrity?.counts?.members, activeExposure: context.exposure, candidateExposureCount: integrity?.exposureCount, checksumVerified: healthy, readerRoleState: "READ_ONLY_TRANSACTION_VERIFIED", selection: context.selection }, { status: healthy ? 200 : 503, headers: { "Cache-Control": "no-store" } })
      }

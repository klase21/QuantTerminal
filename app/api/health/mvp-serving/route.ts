...
export async function GET() {
  try {
    ...
    return await withServingPostgresFacade(async (_facade, context, port) => {
      if (context.selection === "PREVIEW_INACTIVE_CANDIDATE" || context.selection === "PRODUCTION_EXACT_CANDIDATE_DB") {
        ...
        const expectedExposureCount = context.selection === "PREVIEW_INACTIVE_CANDIDATE" && integrity?.retryLineageVerified ? 1 : 0

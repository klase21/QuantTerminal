...
import { isMvp8z2CandidateReviewMode, mvp8z2CandidateReplayHref } from "@/lib/data-platform/mvp-serving/candidateReview"

export default function ReplayPage({ searchParams }: { searchParams?: Record<string, string | string[] | undefined> }) {
  if (!searchParams?.instrument && !searchParams?.symbol && !searchParams?.start && !searchParams?.projection) redirect(isMvp8z2CandidateReviewMode() ? mvp8z2CandidateReplayHref("BTCUSDT") : demoProfile.primary.replayUrl)
  return (
    <TerminalAppShell>

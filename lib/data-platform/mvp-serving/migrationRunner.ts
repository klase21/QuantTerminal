...
export class MvpServingMigrationRunner {
  constructor(private readonly client: MvpServingPostgresClient) { if (client.roleIntent !== "MIGRATION_OWNER") throw new Error("MVP_SERVING_MIGRATION_OWNER_REQUIRED") }
  async apply(appliedBy: string): Promise<readonly MvpServingMigrationResult[]> {
    await this.client.sql.unsafe("CREATE SCHEMA IF NOT EXISTS serving_control")
    await this.client.sql.unsafe("CREATE TABLE IF NOT EXISTS serving_control.migration_ledger (migration_id text PRIMARY KEY,migration_filename text NOT NULL UNIQUE,migration_checksum text NOT NULL CHECK (migration_checksum ~ '^[0-9a-f]{64}$'),applied_at timestamptz NOT NULL,applied_by text NOT NULL)")

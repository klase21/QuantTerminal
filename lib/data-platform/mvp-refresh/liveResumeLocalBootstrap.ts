...
function buildCandidate(input: Parameters<BoundedLiveSlotAdapter["normalizeAndPersistCandidates"]>[0], row: ArchiveRow | ProviderNativeFundingEvent, ordinal: number, state: SlotState): PopulationCandidate {
  ...
  if (input.dataset === "funding") {
    const candidate = createBoundedFundingCandidate(row as ProviderNativeFundingEvent, context.unitId, raw.objectId, raw.createdAt)
    return Object.freeze({ ...candidate, candidateId: `${candidate.candidateId}:generation:${canonicalChecksum({ executionGenerationId: input.executionGenerationId, candidateId: candidate.candidateId })}`, retrievalAttemptId: context.retrievalAttemptId })
  }
  if (input.dataset === "ohlcv") {
    ...
    const candidateId = createCandidateId({ rawManifestId: raw.objectId, sourceObservationId, parserVersion: governance.parser, candidateOrdinal: `${ordinal}:${input.executionGenerationId}` })
  ...
  if (input.dataset === "open-interest") {
    ...
    const candidateId = createCandidateId({ rawManifestId: raw.objectId, sourceObservationId, parserVersion: governance.parser, candidateOrdinal: `${value.sourceOrdinal}:${input.executionGenerationId}` })
  ...
  const candidateId = createCandidateId({ rawManifestId: raw.objectId, sourceObservationId, parserVersion: governance.parser, candidateOrdinal: `0:${input.executionGenerationId}` })
...
export function createDatasetAdapter(input: { readonly dataset: RefreshLogicalDataset; readonly storage: ObjectStoragePort; readonly objectRoot: string; readonly d2Client: IsolatedPostgresClient; readonly d2: CanonicalPersistenceAdapter; readonly d3: PopulationPostgresAdapter; readonly d3Coordinator?: PopulationPostgresAdapter; readonly canonical: D3CanonicalCommitPort; readonly refresh: MvpRefreshStore; readonly allowBtcOhlcvAcquisition?: boolean; readonly enableResumeReconciliation?: boolean; readonly enableCleanRebuildRetainedRawReuse?: boolean }): BoundedLiveSlotAdapter {
  ...
  return Object.freeze({
    ...
    async reconcileResume(invocation) {
      ...
      const retrievalIdentity = `mrret_${canonicalChecksum({ logicalSlotId: invocation.logicalSlotId, executionGenerationId: invocation.executionGenerationId, sourceChecksum: checksum })}`
    ...
    async inspectFinalization(invocation) {
      ...
      const retrievalIdentity = `mrret_${canonicalChecksum({ logicalSlotId: invocation.logicalSlotId, executionGenerationId: invocation.executionGenerationId, sourceChecksum })}`

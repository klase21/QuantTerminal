...
export class MvpConsumerProjectionFacade {
  ...
  private async readView(input: { readonly view: MvpConsumerView; readonly instrument?: MvpConsumerInstrument; readonly start?: string; readonly end?: string; readonly candidateId?: string; readonly projectionVersionId?: string }): Promise<readonly MvpProjectionVersion[]> {
    ...
    if (input.view === "markets") return this.required([await this.source.latest("MacroContextProjection", "GLOBAL_MACRO_CONTEXT"), await this.source.latest("BitcoinEtfFlowProjection", "BTC_SPOT_ETF_US"), ...await this.latestForAll("InstrumentMarketSummaryProjection"), ...await this.latestForAll("SourceLineageSummaryProjection"), ...await this.latestCoverage()])
    ...
    if (input.projectionVersionId) {
      ...
      const pairedKind = input.view === "replay" ? "ResearchEvidenceProjection" as const : "ReplayTimelineProjection" as const
      const paired = (await this.source.list({ kind: pairedKind, subjectId: instrument, start: exact.eventTimeStart, end: exact.eventTimeEnd, exposure: "READY_FOR_CUTOVER", limit: 2 }))[0] ?? null
      return this.required([exact, paired, await this.source.latest("MacroContextProjection", "GLOBAL_MACRO_CONTEXT"), await this.source.latest("BitcoinEtfFlowProjection", "BTC_SPOT_ETF_US"), await this.source.latest("SourceLineageSummaryProjection", instrument), ...await this.latestCoverage(instrument)])
    }
    ...
    const pairedResearch = input.view === "replay" && primary ? (await this.source.list({ kind: "ResearchEvidenceProjection", subjectId: instrument, start: primary.eventTimeStart, end: primary.eventTimeEnd, exposure: "READY_FOR_CUTOVER", limit: 2 }))[0] ?? null : null
    ...
    return this.required([primary, pairedResearch, await this.source.latest("MacroContextProjection", "GLOBAL_MACRO_CONTEXT"), await this.source.latest("BitcoinEtfFlowProjection", "BTC_SPOT_ETF_US"), await this.source.latest("SourceLineageSummaryProjection", instrument), ...annotations, ...await this.latestCoverage(instrument)])
  }

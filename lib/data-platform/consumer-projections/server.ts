...
export async function withMvpConsumerProjectionFacade<T>(work: (facade: MvpConsumerProjectionFacade, context: MvpServingRequestContext) => Promise<T>): Promise<T> {
  ...
  try {
    ...
    return await work(new MvpConsumerProjectionFacade(source, { id: corpus.projectionCorpusId, checksum: corpus.projectionCorpusChecksum }), Object.freeze({ mode: "LOCAL_TRUTH", corpusId: corpus.projectionCorpusId, checksum: corpus.projectionCorpusChecksum, exposure: "CONSUMER_VISIBLE", governedThrough: "2026-07-15T00:00:00.000Z", selection: "ACTIVE_EXPOSURE", runtimeSelectionPolicy: "ACTIVE_ONLY", transactionReadOnly: true }))

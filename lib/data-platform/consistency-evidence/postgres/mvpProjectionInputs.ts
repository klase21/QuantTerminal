...
export async function loadMvpProjectionEvidenceInputs(input: { readonly corpus: MvpProjectionInputCorpus; readonly d4: ConsistencyPostgresRuntime; readonly d2: IsolatedPostgresClient; readonly d3: D3PostgresClient; readonly objectRoot: string; readonly eventTimeStart?: string; readonly eventTimeEnd?: string; readonly instruments?: readonly string[] }): Promise<readonly MvpProjectionEvidenceInput[]> {
  ...
  const [results, facts, coverage, prices] = await Promise.all([
    ...
    input.d3.sql.unsafe<Array<{ decision_id: string; bounded_dimensions: Record<string, unknown> }>>("SELECT decision_id,bounded_dimensions FROM coverage.watermark_eligibility_decisions WHERE eligibility_result='ELIGIBLE' ORDER BY decision_id"),
  ...
  return Object.freeze(windows.map((window): MvpProjectionEvidenceInput => {
    ...
    const date = assessment.eventTimeStart.slice(0, 10), coverageDecisionIds = coverage.filter((row) => { const dimensions = JSON.stringify(row.bounded_dimensions); return dimensions.includes(assessment.instrument) && dimensions.includes(date) }).map((row) => row.decision_id)

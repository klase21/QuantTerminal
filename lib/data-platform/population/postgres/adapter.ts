...
export interface PopulationLineageAudit {
  readonly units: readonly { readonly logicalSlotId: string; readonly unitId: string; readonly runId: string; readonly executionAttemptIds: readonly string[]; readonly dataset: string; readonly instrument: string; readonly state: string; readonly resumeStage: PopulationResumeStage; readonly fencingToken: number; readonly activeLease: boolean; readonly leaseExpired: boolean; readonly checkpointId: string | null; readonly retrievalAttempts: number; readonly candidates: number; readonly conflicts: number; readonly nonAuthoritativePartialOutputs: number; readonly resumeEligible: boolean }[]
...
export function createPopulationPostgresAdapter(client: D3PostgresClient): PopulationPostgresAdapter {
  const lockImmutableIdentity = async (sql: postgres.TransactionSql, scope: string, identity: string) => {
    await sql`SELECT pg_advisory_xact_lock(hashtextextended(${`${scope}:${identity}`},0))`
  }
  ...
  const appendSubmissionEvent = async (
    ...
    await lockImmutableIdentity(sql, "canonical-submission-event", eventId)
    const existing = await sql<{ readonly event_checksum: string }[]>`SELECT event_checksum FROM population.canonical_submission_events WHERE event_id=${eventId}`
    if (existing[0]) {
  ...
  const appendImmutableUnitEvent = async (sql: postgres.TransactionSql, input: PopulationUnitEventInput): Promise<PopulationUnitEventResult> => {
    ...
    await lockImmutableIdentity(sql, "population-unit-event", input.eventId)
    const rows = await sql<Record<string, unknown>[]>`SELECT event_id,unit_id,run_id,event_type,previous_state::text,next_state::text,fencing_token,actor_id,occurred_at,details FROM control.population_unit_events WHERE event_id=${input.eventId}`
  ...
  const persistBounded = async (sql: postgres.TransactionSql, input: PersistBoundedAcquisitionInput | BoundedAcquisitionProbeInput, enforceLease: boolean): Promise<PersistBoundedAcquisitionResult> => {
    ...
    await lockImmutableIdentity(sql, "retrieval-attempt", attempt.attemptId)
    const existingAttempts = await sql<Record<string, unknown>[]>`SELECT * FROM control.retrieval_attempts WHERE attempt_id=${attempt.attemptId}`
    ...
    if (input.candidates.length) {
      ...
      for (const candidateId of candidateIds) await lockImmutableIdentity(sql, "population-candidate", candidateId)
  ...
  const recordFailure = async (input: { readonly unitId: string; readonly leaseId: string; readonly ownerId: string; readonly fencingToken: number; readonly classification: string; readonly at: string }, failureKind: "LINEAGE" | "CANONICAL"): Promise<PopulationUnitEventResult> => client.transaction(async (sql) => {
    ...
    await lockImmutableIdentity(sql, "population-unit-event", failureId)
    const existing = await sql<Record<string, unknown>[]>`SELECT event_id,unit_id,run_id,event_type,previous_state::text,next_state::text,fencing_token,actor_id,occurred_at,details FROM control.population_unit_events WHERE event_id=${failureId}`
    if (existing[0]) {
      const stable = normalizedEvent(existing[0]), expectedSubset = { unitId: input.unitId, eventType: "STAGE_FAILURE", nextState: "RETRYABLE", fencingToken: input.fencingToken, actorId: input.ownerId, occurredAt: new Date(Math.floor(Date.parse(input.at) / 1_000) * 1_000).toISOString(), classification: input.classification }
  ...
  const adapter: PopulationPostgresAdapter = {
    ...
    async auditBoundedAcquisitionLineage(intervalStart, intervalEnd, requestedBy) {
      const units = await client.sql<{ readonly logical_slot_id: string; readonly unit_id: string; readonly run_id: string; readonly execution_attempt_ids: string[]; readonly dataset_id: string; readonly subject_or_symbol: string; readonly current_state: string; readonly current_fencing_token: number; readonly active_lease_id: string | null; readonly expires_at: string | null; readonly released_at: string | null; readonly current_checkpoint_id: string | null; readonly retrieval_attempts: number; readonly candidates: number; readonly committed_outcomes: number; readonly conflicts: number }[]>`SELECT u.partition_key logical_slot_id,u.unit_id,r.run_id,(SELECT array_agg(pr.run_id ORDER BY pr.attempt_number,pr.run_id) FROM control.population_runs pr WHERE pr.job_id=j.job_id) execution_attempt_ids,u.dataset_id,u.subject_or_symbol,u.current_state::text,u.current_fencing_token,u.active_lease_id,l.expires_at,l.released_at,u.current_checkpoint_id,(SELECT count(DISTINCT a.attempt_id)::int FROM control.retrieval_attempts a WHERE a.unit_id=u.unit_id) retrieval_attempts,(SELECT count(DISTINCT c.candidate_id)::int FROM population.candidates c WHERE c.unit_id=u.unit_id) candidates,(SELECT count(DISTINCT o.candidate_id)::int FROM control.population_outcomes o WHERE o.unit_id=u.unit_id AND o.outcome_kind IN ('COMMITTED','DUPLICATE')) committed_outcomes,(SELECT count(*)::int FROM population.candidate_conflicts x JOIN population.candidates c ON c.candidate_id=x.candidate_id WHERE c.unit_id=u.unit_id) conflicts FROM control.population_units u JOIN control.population_jobs j ON j.job_id=u.job_id JOIN LATERAL (SELECT pr.run_id FROM control.population_runs pr WHERE pr.job_id=j.job_id ORDER BY pr.attempt_number DESC,pr.run_id DESC LIMIT 1) r ON true LEFT JOIN control.population_leases l ON l.lease_id=u.active_lease_id WHERE j.requested_by=${requestedBy} AND u.window_start=${intervalStart} AND u.window_end=${intervalEnd} ORDER BY u.dataset_id,u.subject_or_symbol,u.unit_id`
      const mapped = Object.freeze(units.map((row) => { const attempts = Number(row.retrieval_attempts), candidates = Number(row.candidates), outcomes = Number(row.committed_outcomes); const resumeStage: PopulationResumeStage = attempts === 0 ? "SOURCE_ACQUISITION" : candidates === 0 ? "CANDIDATE_LINEAGE" : outcomes >= candidates ? "COMPLETE" : "CANONICAL_COMMIT"; const activeLease = Boolean(row.active_lease_id && !row.released_at && row.expires_at && Date.parse(row.expires_at) > Date.now()); return Object.freeze({ logicalSlotId: row.logical_slot_id, unitId: row.unit_id, runId: row.run_id, executionAttemptIds: Object.freeze([...(row.execution_attempt_ids ?? [])]), dataset: row.dataset_id, instrument: row.subject_or_symbol, state: row.current_state, resumeStage, fencingToken: Number(row.current_fencing_token), activeLease, leaseExpired: Boolean(row.active_lease_id && !row.released_at && row.expires_at && Date.parse(row.expires_at) <= Date.now()), checkpointId: row.current_checkpoint_id, retrievalAttempts: attempts, candidates, conflicts: Number(row.conflicts), nonAuthoritativePartialOutputs: Math.max(0, (row.execution_attempt_ids?.length ?? 1) - 1), resumeEligible: !activeLease && resumeStage !== "COMPLETE" }) }))
    ...
    async recordIntermediateD2Result(input) {
      ...
      await client.transaction(async (sql) => {
        ...
        await lockImmutableIdentity(sql, "population-outcome", input.outcomeId)

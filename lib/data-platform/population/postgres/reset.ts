...
export async function resetD3Schemas(client: D3PostgresClient, command: D3ResetCommand): Promise<void> {
  ...
  await client.transaction(async (sql) => {
    const functions = await sql<{ readonly signature: string }[]>`SELECT p.oid::regprocedure::text signature FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='control' AND p.proname IN ('advance_population_unit','heartbeat_population_lease','claim_population_unit')`
    for (const fn of functions) await sql.unsafe(`DROP FUNCTION ${fn.signature} CASCADE`)

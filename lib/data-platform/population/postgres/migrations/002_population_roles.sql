...
GRANT INSERT, SELECT ON population.candidates, population.candidate_conflicts, population.canonical_submissions TO qt_d3_worker;
...
GRANT EXECUTE ON FUNCTION control.heartbeat_population_lease(text,text,text,bigint,timestamptz,timestamptz) TO qt_d3_worker;
GRANT EXECUTE ON FUNCTION control.advance_population_unit(text,text,text,bigint,control.population_unit_state,text,timestamptz) TO qt_d3_worker;
...
GRANT SELECT ON population.candidates, population.candidate_conflicts, population.canonical_submissions, quality.candidate_validation_results, quality.candidate_evaluation_runs, quality.candidate_evaluation_results, coverage.watermark_eligibility_decisions TO qt_d3_read_only;

...
function formatNumber(input: unknown, options: { percent?: boolean; decimal?: boolean } = {}) {
  const parsed = Number(input)
  if (!Number.isFinite(parsed)) return text(input)
  const formatted = new Intl.NumberFormat("en-US", { maximumFractionDigits: options.decimal ? 8 : 2 }).format(parsed)
...
function ReasonList({ values, empty = "No governed observations were supplied." }: { values: unknown; empty?: string }) {
  ...
  return items.length ? <ul className="grid gap-2 text-[var(--qt-type-body-size)] text-[var(--qt-color-text-primary)]">{items.map((item) => <li className="flex min-w-0 items-start gap-2" key={item}><span aria-hidden="true" className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-current opacity-60" /><span className="min-w-0 break-words">{label(item)}</span></li>)}</ul> : <p className="text-[var(--qt-type-body-size)] text-[var(--qt-color-text-muted)]">{empty}</p>
}
...
export default function MvpCutoverPage({ view }: { view: MvpView }) {
  ...
  const body = view === "dashboard" ? <Dashboard payload={payload} href={href} /> : view === "markets" ? <Markets projections={projections} href={href} /> : view === "scanner" ? <Scanner payload={payload} href={href} /> : view === "trade" ? <Trade payload={payload} href={href} /> : view === "replay" ? <Replay payload={payload} cursor={cursor} onCursor={setCursor} href={href} instrument={instrument} /> : <Research payload={payload} />

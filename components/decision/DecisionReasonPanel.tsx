      ...
      <div className="rounded-3xl border border-zinc-800 bg-black/50 p-4">
        ...
        <div className="space-y-2">
          {decision.reason.map((reason, index) => (
            <div key={reason} className="rounded-2xl border border-zinc-900 bg-zinc-950/70 p-3 text-sm text-zinc-300">
        ...
        <div className="space-y-2">
          {decision.suppress.length ? (
            decision.suppress.map((item) => (
              <div key={item} className="rounded-2xl border border-yellow-300/10 bg-yellow-400/5 p-3 text-sm text-zinc-300">
                {item}
              </div>
            ))
          ) : (
